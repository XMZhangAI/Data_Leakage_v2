...
Adapted from a HuggingFace tran sformers example for training seq2seq models.

Assumes that CodeGen model checkpoints are stored in {data_args.model_repo}/codegen-[6B|16B]-mono.
"""
import os
# FIXME:
...
class SaveModelCallback(TrainerCallback):
    ...
    def on_epoch_end(self, args, state, control, model, **kwargs):
        ...
        if math.floor(state.epoch) < 100: 
            save_path = os.path.join(args.output_dir, 'checkpoint-epoch{}'.format(math.floor(state.epoch)))
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            model.save_pretrained(save_path)


def load_model_tokenizer(model_args, data_args, last_checkpoint, device):
    ...
    if "Llama" in model_args.model_name_or_path:
        if last_checkpoint is not None:
            model = AutoModelForCausalLM.from_pretrained(
                last_checkpoint,
                device_map={"": data_args.gpu[0]}
            )
        else:
            model = AutoModelForCausalLM.from_pretrained(
                f"{data_args.model_repo}/{model_args.model_name_or_path}",
                device_map={"": data_args.gpu[0]}
            )
        ...
        tokenizer = AutoTokenizer.from_pretrained(f"{data_args.model_repo}/{model_args.model_name_or_path}", trust_remote_code=True)
        ...
        if last_checkpoint is not None:
            ...
            model = modeling_codegen.CodeGenForCausalLM.from_pretrained(
                f"{data_args.model_repo}/{model_args.model_name_or_path}-mono",
...
def main():
    ...
    if torch.cuda.is_available():  
        device = [f"cuda:{gpu}" for gpu in data_args.gpu]
    else:  
    ...
    model, tokenizer = load_model_tokenizer(model_args, data_args, last_checkpoint, device)

    # FIXME: Load LoRA configuration
    ...
    def preprocess_example(example):
        ...
        return {
            "input_ids": torch.IntTensor(input_ids).to(device[0]),
            "labels": torch.IntTensor(labels_input_ids).to(device[0]),
        }
