    ...
    tokenizer.padding_side = "right"
        ...
        if last_checkpoint is not None:
            ...
            model = AutoModelForCausalLM.from_pretrained(
                ...
                cache_dir="/data3/public_checkpoints/huggingface_models/codegen-6b-multi",  # 指定缓存目录为model_path
        ...
        if last_checkpoint is not None:
            ...
            model = AutoModelForCausalLM.from_pretrained(
                ...
                # low_cpu_mem_usage=True,
                cache_dir="/data3/public_checkpoints/huggingface_models/bloom-7b",  # 指定缓存目录为model_path
                trust_remote_code=True
            )
        ## IMPORTANT: DO NOT REMOVE
        model = model.to('cuda', dtype=torch.float32)
        ...
        tokenizer = AutoTokenizer.from_pretrained(f"bigscience/{model_name}",
                # low_cpu_mem_usage=True,
                cache_dir="/data3/public_checkpoints/huggingface_models/bloom-7b", trust_remote_code=True)
        tokenizer.pad_token = tokenizer.eos_token
        tokenizer.padding_side = "right"
